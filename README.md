...
### Block of Candles (BoC) – FULLY FIXED ALL TF
  ...
  optionally drawing mitigation zones for future retests. It also offers MTF view, stats table (avg length, range, bullish/bearish breaks) and alerts for block close,
  directional breakout and long consolidations.
  ...
  **Note:** Blocks with a higher candle count typically represent stronger consolidation zones with better-tested levels,
    making their mitigation zones more significant for entries. Consider using block length as a scoring/weighting factor when prioritizing setups.
